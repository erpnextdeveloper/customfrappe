...
class CustomizeForm(Document):
    ...
    def transfer_to_doc(self):
        property_setter = frappe.db.sql("""select field_name,value from `tabProperty Setter` where doc_type='Item' and property='label' and field_name in 
            ('l1','l2','l3','l4','l5','l6','l7','l8','l9','l10','l11','l12','l13','l14','l15','l16','l17','l18','l19','l20') order by idx""")
        for i in property_setter:
            frappe.db.sql("update `tabDocField` set label='{0}' where parent='Item' and fieldname='{1}'".format(i[1],i[0]))
            frappe.db.sql("update `tabPrice List` set name='{0}' where fieldname='{1}'".format(i[1],i[0]))
            frappe.db.sql("update `tabCustomer` set default_price_list='{0}' where fieldname ='{1}'".format(i[1],i[0]))
            frappe.db.commit()
